  ...
  return(
    <div className="w-64 h-[calc(100vh-61px)] bg-slate-200 border-r border-gray-200/50 p-5 sticky top-[61px] z-20 ">
      ...
      { user?.profileImageUrl ? (
        <img src={user.profileImageUrl} alt="progile pic" className="w-20 h-20 rounded-full bg-slate-400"/>
      ): (<></>)}
      <h5 className="text-gray-950 leading-6 font-medium ">{user?.fullName || ""}</h5>
    </div>
    { SIDE_BAR_DATA.map((item, index)=>(
      <button key={`menu_${index}`} className={`w-full flex items-center gap-4 text-[15px] ${activeMenu === item.label ? "text-white bg-secondary" : ""} py-3 px-6 rounded-lg mb-3`} 
